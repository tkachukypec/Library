...
namespace LibraryIS.ViewModels
{
    class ReportViewModel : ViewModelBase
    {
        ...
        public ReportViewModel()
        {
            ...
            BookLabels = pubs.Select(p => p.Book.Title).ToList();
            CountExtraditionValues = new List<double>();
            int iCount = 0;
            pubs.ForEach(p =>
            {
                CountExtraditionValues.Add(DataBase.GetEntities().BookIssuance.Where(e => e.PublicationId == p.Id).Count());
                iCount++;
            });
            ...
            PopularPub = pubs.FirstOrDefault(p => p.BookIssuance.Count() == CountExtraditionValues.Max());
        }
    }
}
